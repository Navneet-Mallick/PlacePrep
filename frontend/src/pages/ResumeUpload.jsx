...
export default function ResumeUpload() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Upload Form */}
      <form onSubmit={handleSubmit} className="rounded-lg border border-neutral-800 bg-neutral-900/50 p-5">
        <label className="block text-xs font-medium text-neutral-400 mb-2">Resume file (PDF or DOCX)</label>
        <div className="flex items-center gap-3">
          <input
            type="file"
            accept=".pdf,.docx"
            onChange={(e) => setFile(e.target.files?.[0] ?? null)}
            className="text-sm text-neutral-400 file:mr-3 file:rounded-md file:border-0 file:bg-neutral-800 file:px-3 file:py-2 file:text-sm file:text-white file:cursor-pointer hover:file:bg-neutral-700"
          />
        </div>
        {file && <p className="mt-2 text-xs text-neutral-500">{file.name} ({(file.size / 1024).toFixed(0)} KB)</p>}
        <button
          type="submit"
          disabled={loading || !file}
          className="mt-4 w-full py-2.5 bg-white text-neutral-900 text-sm font-medium rounded-md hover:bg-neutral-200 disabled:opacity-50 disabled:cursor-not-allowed"
        >
        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="p-4 rounded-lg border border-neutral-800 bg-neutral-900/50">
              ...
              <p className="text-3xl font-bold text-white mt-1">{result.resume_score}<span className="text-lg text-neutral-500">/100</span></p>
            </div>
            <div className="p-4 rounded-lg border border-neutral-800 bg-neutral-900/50">
              <p className="text-xs text-neutral-500">Predicted Role</p>
          ...
          {result.entities && (
            <div className="rounded-lg border border-neutral-800 bg-neutral-900/50 p-5 space-y-4">
              <h2 className="text-sm font-medium text-neutral-300">Extracted Information</h2>
              
              {result.entities.skills?.length > 0 && (
                <div>
                  <p className="text-xs text-neutral-500 mb-2">Skills ({result.entities.skills.length})</p>
                  <div className="flex flex-wrap gap-1.5">
                    {result.entities.skills.map((skill) => (
                      <span key={skill} className="px-2 py-0.5 rounded text-xs bg-neutral-800 text-neutral-300 border border-neutral-700">
                        {skill}
                      </span>
                    ))}
                  </div>
              ...
              {result.entities.email?.length > 0 && (
                <p className="text-xs text-neutral-500">Contact: {result.entities.email.join(', ')}</p>
              )}
            </div>
          )}
          ...
          {result.suggestions?.length > 0 && (
            <div className="rounded-lg border border-neutral-800 bg-neutral-900/50 p-5">
              <h2 className="text-sm font-medium text-neutral-300 mb-3">Suggestions</h2>
          ...
          {hasRecommendations && (
            <div className="rounded-lg border border-neutral-800 bg-neutral-900/50 p-5 space-y-3">
              <h2 className="text-sm font-medium text-neutral-300">AI Recommendations</h2>
              {rec.summary && <p className="text-sm text-neutral-400">{rec.summary}</p>}
          ...
          {!hasRecommendations && (
            <p className="text-xs text-neutral-600">AI recommendations unavailable (Gemini API quota may be exhausted).</p>
          )}
        </div>
      )}
    </div>
    ...
    <div>
      ...
      <ul className="space-y-0.5">
        {items.slice(0, 4).map((item, i) => (
          <li key={i} className="text-sm text-neutral-400 truncate">{item}</li>
        ))}
      </ul>
    </div>
  )
